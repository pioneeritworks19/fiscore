...
function ViolationsPage({ tenantId, sites }: { tenantId: string; sites: Site[] }) {
  const { t } = useTranslation();
...
function ViolationListRow({ violation, onOpen }: { violation: Violation; onOpen: () => void }) {
  const { t } = useTranslation();
...
function NotificationHistoryPage({ tenantId }: { tenantId: string }) {
  const { t } = useTranslation();
  ...
  const filteredEvents = events.filter(
    (event) => eventFilter === 'all' || event.eventType === eventFilter,
  );
...
function SettingsPage({ user, profile }: { user: User; profile: UserProfile | null }) {
  const { t } = useTranslation();
