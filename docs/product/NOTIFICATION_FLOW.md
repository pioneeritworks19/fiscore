...
firebase functions:secrets:set RESEND_WEBHOOK_SECRET --project fiscore-dev
```
...
Resend provider delivery status is captured through the `receiveResendWebhook`
HTTP function. The webhook must be configured in Resend with the Firebase
function URL and the Resend signing secret must be stored in
`RESEND_WEBHOOK_SECRET`. The webhook records the provider event id for
idempotency and updates the matching notification event and delivery attempt by
Resend message id.

For Firebase dev, the expected webhook URL is:

```text
https://us-central1-fiscore-dev.cloudfunctions.net/receiveResendWebhook
```

Recommended Resend events for V1 are `email.sent`, `email.delivered`,
`email.delivery_delayed`, `email.bounced`, and `email.complained`. Open/click
events can be enabled later if FiScore decides they are useful for support, but
they are not required for account access troubleshooting.
...
When Resend is enabled, `providerDeliveryStatus` adds mailbox/provider outcome
detail such as `delivered`, `delayed`, `bounced`, `complained`, `opened`, or
`clicked`. FiScore keeps `status` as the business send state and uses
`providerDeliveryStatus` for delivery troubleshooting.
...
Passwordless sign-in links are generated by Firebase Auth on the backend and
sent through the notification email adapter. If the email matches a tenant invite
or active member, the notification record is tenant-scoped; otherwise it is a
global pre-tenant account-access event.
